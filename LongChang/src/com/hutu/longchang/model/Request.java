...
public class Request {
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			if (Connect.NET_WORK_ERROR == msg.what) {
				// Log.d(MainActivity.CITY_HUNTER_TAG, "network error" +
				// msg.what);
			...
			// JSONObjectExt jsonObj = new
			// JSONObjectExt(msg.getData().getString(
			// Connect.RESPONSE));
			// int serverCode = jsonObj.getInt("respcode");
			// if (Connect.RESPONSE_CODE_TOKEN_EXPIRE == serverCode) {
			// // Log.d(MainActivity.CITY_HUNTER_TAG,
			// "token expire: "+msg.getData().getString(Connect.RESPONSE));
			// // if
			// (com.cityhunter.model.social.MyGoods.getInstance().getProgressbardialog()
			// != null) {
			// //
	 ...
	 *            文件
	 */
	public void sendHttpPostRequest(final String url,
			final HashMap<String, ?> map, final Object file) {
				...
				public void run() {
					...
					while (runStep < 3) {
						...
						if (file instanceof Bitmap) {
							...
							Log.d("CODE_2",
									"分享==" + url + new String(map.toString()));
	...
	public void jinsichaxun(String TabNum, String typeName, String type,
	 ...
	 *            申请人名称(英文)
	 */
	public void zongHeInfo(String TabNum, String RegNO, String SPName,
			String SQNameCN, String SQNameEN) {
	...
	public void zongHeInfos(String type, String RegNO,
			String Sbmc, String applicant, String Intcls, int start,
			int end) {
		String url = HostName + "?method=SBZongHeInfo";
//		if (TabNum != null && TabNum.length() > 0) {
//			url += ("&TabNum=" + TabNum);
//		}syso
		...
		if(Sbmc != null && Sbmc.length() > 0){
			try {
				Sbmc = URLEncoder.encode(Sbmc,"UTF-8");
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
			url += ("&Sbmc=" + Sbmc);
		...
		if(applicant != null && applicant.length() > 0){
			try {
				applicant = URLEncoder.encode(applicant,"UTF-8");
			} catch (UnsupportedEncodingException e) {
	...
	public void stateFlowInfo(String TabNum, String RegNO) {
		String url = HostName + "?method=SBStateFlowInfo&RegNO=" + RegNO
				+ "&TabNum=" + TabNum;
		httpGetConnect(url);
	}
	...
	public void setRegisterUser(String account, String password) {
		String url = HostName + "?method=SetRegisterUser&UserName=" + account
				+ "&PassWord=" + password;
		httpGetConnect(url);
	}
	...
	public void getLognUser(String account, String password) {
		String url = HostName + "?method=GetLognUser&UserName=" + account
				+ "&PassWord=" + password;
		httpGetConnect(url);
	}
